    ...
    <div className={`flex flex-col lg:flex-row gap-8 lg:gap-12 items-center ${reverse ? "lg:flex-row-reverse" : ""}`}>
      ...
      <div className="w-full lg:w-1/2">
        <div className="relative rounded-2xl overflow-hidden shadow-2xl group">
          ...
          <div className="absolute bottom-4 sm:bottom-6 left-4 sm:left-6 bg-white/95 backdrop-blur px-4 sm:px-5 py-2 sm:py-3 rounded-xl shadow-lg">
            <p className="text-xs text-slate-500 uppercase font-semibold">{t("servicePage.performance")}</p>
      ...
      <div className="w-full lg:w-1/2">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900">{service.title}</h2>
        </div>

        <p className="text-sm sm:text-base md:text-lg text-slate-600 mb-5 sm:mb-6">{service.description}</p>

        <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 mb-5 sm:mb-6">
          <h3 className="font-semibold text-slate-900 mb-3 sm:mb-4">{t("servicePage.keyFeatures")}</h3>
