...
export default function AboutSection({ title, paragraphs, cards, image }: Props) {
  return (
    <section className="bg-white py-10 sm:py-12">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 items-start">
          ...
          <div>
            ...
            {paragraphs.map((text, i) => (
              <p key={i} className="text-sm sm:text-base md:text-lg text-slate-600 mb-4 sm:mb-6 leading-relaxed">
                {text}
              </p>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
            {cards.map((card, index) => (
              <div
                key={index}
                className="bg-slate-50 p-5 sm:p-8 rounded-2xl border border-slate-100"
              >
                <div className={`w-10 h-10 flex items-center justify-center rounded-full mb-3 ${
                  card.icon === "trust" ? "bg-red-100 text-red-500" : "bg-blue-100 text-blue-500"
                }`}>
                  {card.icon === "trust" ? (
                ...
                <h3 className="text-base sm:text-xl font-bold text-slate-900 mb-2">{card.title}</h3>
        ...
        <div className="mt-8 sm:mt-10">
          <div className="relative w-full h-[220px] sm:h-[400px] md:h-[600px] lg:h-[850px] rounded-xl overflow-hidden shadow-md">
